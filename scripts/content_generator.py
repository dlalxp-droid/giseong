...
대주제(theme): generate.py의 THEME_POOL에 등록된 주제 중 하나.
세부주제(sub_topic): 선택적. 주어지면 그 각도로 작성.
카드 구조: cover → intro → point×3 → tip → summary → cta
...
주어진 '대주제'와 (있을 경우) '세부주제'에 맞춰, 밴드 구독자가 저장하고
싶어지는 8장짜리 정보 카드뉴스를 만듭니다.
...
- 특정 보험사·상품·금융 종목·병원·약품·앱·서비스 이름을 언급하지 마세요.
- 의료 진단/처방, 투자 권유, 법률 단정 금지. "정확한 건 전문가·기관 확인"으로 안내.

# 대주제별 방향 (참고 — 주어진 주제에 맞춰 일반 정보 관점에서 작성)
- 보험 정보: 소비자가 알아두면 좋은 보험 일반 상식
- 건강 정보: 일상 속 건강 습관·상식 (진단 아님)
- 생활 정보: 절약·생활 꿀팁·실용 정보
- 금융 정보: 금융 기초 상식·재테크 개념 (종목 추천 아님)
- 노후·은퇴 정보: 노후 준비 일반 상식
- 부동산·주택 정보: 임대차·주거 관련 일반 상식
- 자녀·가족 정보: 가족 운영·자녀 양육 일반 정보
- 자기계발 정보: 일상 자기관리·습관 형성
그 외 주제는 같은 톤으로 일반 정보 관점에서 작성하세요.

# 세부주제(sub_topic)가 주어지면
그 각도로 구체적으로 작성하세요. 같은 대주제라도 세부주제가 다르면 완전히
다른 콘텐츠가 되어야 합니다. 세부주제 안에서 흔한 클리셰는 피하고, 한두
걸음 더 들어간 실용 정보 위주로.
...
def generate_card_set(
    ...
    """
    Claude API 호출 → 정보형 카드뉴스 한 세트 생성.

    Args:
        theme: 대주제 (e.g. "보험 정보", "건강 정보" ...)
        sub_topic: 세부주제 (e.g. "실손보험 자기부담금 점검"). 주어지면 그 각도로.
    ...
    parts = []
    if theme:
        parts.append(f"대주제: {theme}")
    else:
        parts.append("대주제: 자유 선택")
    if sub_topic:
        parts.append(f"세부주제: {sub_topic} (이 각도로 구체적으로 작성)")
    parts.append(f"카드 수: {cards_per_set}장")
    parts.append("위 시스템 프롬프트 형식대로 JSON만 출력해주세요.")
    user_msg = "\n".join(parts)
...
def stub_card_set(theme: str = "보험 정보") -> CardSet:
    return CardSet(
        ...
        cards=[
            {"type": "cover", "label": theme, "headline": "실손보험, 자기부담금부터 확인하세요",
             "accent": "자기부담금", "subhead": "가입만 해두고 내용을 모르면 정작 필요할 때 손해를 봅니다."},
            {"type": "intro", "label": "왜 중요할까", "headline": "같은 보험도 조건따라 다릅니다",
             "accent": "", "subhead": "가입 시기·유형에 따라 보장 범위와 부담금이 달라질 수 있습니다."},
            {"type": "point", "label": "01", "headline": "자기부담금을 확인하세요",
             "body": "병원비 중 본인이 내는 부분입니다. 가입 시기에 따라 비율이 다른 경우가 많습니다."},
            {"type": "point", "label": "02", "headline": "갱신형·비갱신형 구분",
             "body": "갱신형은 시간이 지나면 보험료가 오를 수 있습니다. 유형을 꼭 확인하세요."},
            {"type": "point", "label": "03", "headline": "중복가입은 실익이 적습니다",
             "body": "실손은 여러 개 들어도 실제 쓴 만큼만 나눠 보장됩니다. 중복 점검이 필요합니다."},
            {"type": "tip", "label": "실전 TIP", "headline": "1년에 한 번은 점검하세요",
             "body": "보장 내용·갱신 주기·자기부담금을 연 1회 확인하면 불필요한 지출을 줄일 수 있습니다."},
            {"type": "summary", "label": "핵심 요약", "headline": "알고 가입하면 손해 안 봅니다",
             "accent": "알고", "subhead": "자기부담금 · 갱신 여부 · 중복 — 이 3가지만 기억하세요."},
            {"type": "cta", "label": "저장 & 공유", "headline": "이 카드, 가족과 나눠보세요",
...
if __name__ == "__main__":
    ...
    p.add_argument("--theme", default=None, help="대주제")
    p.add_argument("--sub", default=None, help="세부주제")
    p.add_argument("--stub", action="store_true")
    p.add_argument("--out", default=None)
    args = p.parse_args()

    cs = (
        stub_card_set(args.theme or "보험 정보")
        if args.stub
        else generate_card_set(args.theme, sub_topic=args.sub)
    )
